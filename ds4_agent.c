...
static agent_tool_syntax agent_tool_syntax_for_engine(ds4_engine *engine) {
    ...
    return ds4_engine_is_deepseek41(engine) ? AGENT_TOOL_SYNTAX_DSML41
                                           : AGENT_TOOL_SYNTAX_DSML;
}
...
static const char *agent_tool_start(agent_tool_syntax syntax) {
    if (syntax == AGENT_TOOL_SYNTAX_GLM || syntax == AGENT_TOOL_SYNTAX_QWEN ||
        syntax == AGENT_TOOL_SYNTAX_LAGUNA) return "<tool_call>";
...
static void agent_worker_maybe_append_system_prompt_reminder(agent_worker *w) {
    ...
    agent_trace(w, "system prompt reminder injected at transcript=%d",
                w->transcript.len);
    if (agent_tool_syntax_is_tagged(agent_tool_syntax_for_engine(w->engine)) ||
        agent_syntax_is_xml_tool_call(agent_tool_syntax_for_engine(w->engine))) {
...
static bool agent_tool_value_close_tail(agent_tool_syntax syntax,
    ...
    if (syntax == AGENT_TOOL_SYNTAX_GLM || syntax == AGENT_TOOL_SYNTAX_LAGUNA)
...
static void agent_dsml_finish(agent_dsml_parser *p) {
    ...
    if ((!agent_tool_syntax_is_tagged(p->syntax) &&
         !agent_syntax_is_xml_tool_call(p->syntax)) || !p->glm_after_call)
        return;
...
static void agent_dsml_parse(agent_dsml_parser *p) {
    ...
    if (p->syntax == AGENT_TOOL_SYNTAX_GLM || p->syntax == AGENT_TOOL_SYNTAX_LAGUNA) {
...
static bool agent_stream_dsml_start_match(agent_tool_syntax syntax,
    ...
    if (agent_tool_syntax_is_tagged(syntax) || agent_syntax_is_xml_tool_call(syntax)) {
    ...
    for (int tool_round = 0; ; tool_round++) {
                    ...
                    dsml.state == AGENT_DSML_PARAM_VALUE))
        {
            ...
            snprintf(dsml.error, sizeof(dsml.error),
                     ...
                     agent_tool_syntax_is_tagged(tool_syntax) ? "incomplete tagged tool call" :
